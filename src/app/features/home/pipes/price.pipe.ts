...
import { Price, CompPriceList } from 'app/shared/models'
...
export class PricePipe implements PipeTransform {
  transform(
  ...
  ): Nullable<number | Price['taxes_and_fees'] | CompPriceList> {
    ...
    switch (type) {
      ...
      case 'compList': {
        if (!priceObj.competitors) return null
        const priceList: CompPriceList = Object.keys(priceObj.competitors).map(key => ({
          name: key,
          price: priceObj.competitors![key] as number
        }))
